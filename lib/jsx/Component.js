...
export default class Component {
  ...
  refMap (k, id, { multiple = false } = {}) {
    ...
    const arr = map.get(id) || (map.set(id, []).get(id))
    const index = arr.length
    arr[index] = null

    return function updateRefMapArray (el) {
      arr[index] = el
    }
  }

  watch (signals, fn, { immediate = false } = {}) {
    const arr = Array.isArray(signals) ? signals : [signals]
    for (const signal of arr) {
      // dispatchListeners already ran if mounted, so subscribe immediately; otherwise it will be called by dispatchListeners after mount
      if (this.mounted) signal.subscribe(fn)
      this._collector.storeEvents.push({ store: signal, fn, init: false })
    }
    if (immediate) fn(arr.length === 1 ? arr[0].value : arr.map(s => s.value))
  }
