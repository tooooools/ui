...
const updateClass = (n, el) => v => el.classList.toggle(n, v || false)
const updateAttrib = (name = 'textContent', isSvg, el) => v => {
...
function setDomAttrib (el, name, value, collector, isSvg) {
  ...
  if (name === 'ref') {
    ...
    collector.append({ storeEvents: [{ store: value, init: true, fn }] })
    ...
    // A bare object (not wrapped in an array) is treated as a single entry, e.g. class={{ 'c': true }}
        ...
        if (typeof v === 'object') {
          for (const k in v) {
            ...
            if (isSignal(v[k])) {
              ...
              collector.append({ storeEvents: [{ store: v[k], init: true, fn }] })
            }
          }
        }
      }
    ...
    if (typeof value === 'object') {
      for (const key in value) {
        ...
        if (isSignal((value ?? [])[key])) {
          collector.append({ storeEvents: [{ store: value[key], init: true, fn: updateProp }] })
        } else {
    ...
    try { updateAttrib(name, isSvg, el)(value) } catch (e) { /* */ }
